...
***Test positivity*** is the ratio of positive tests to all tests reported in a specific time frame (usually 1 day). For example, daily test positivity is calculated by dividing the number of positive tests reported each day (***Confirmed Cases***) by the total number of tests carried out on that day (***Tests Completed***). 
...
*Hospitalization rate is important* because it directly measures health outcomes of interest and is not subject to the most of the measurement problems that arise with case counts and related variables (e.g., Test Positivity).
...
The map files used in Figure \@ref(fig:map-cumcases) were downloaded from the [Map and Geographic Information Center at the UConn Library](http://magic.lib.uconn.edu/).
...
All other things being equal, you would expect larger towns to have more cases than smaller towns, and that comes out pretty clearly in Figures \@ref(fig:town-rate) and \@ref(fig:town-by-county-rate). Figure \@ref(fig:town-by-pop-rate) divides Connecticut's towns up by size, grouping towns of similar size together. This let's us see how each town is doing relative to other towns of similar size. 

```{r town-by-pop-rate, fig.height=10.5, fig.cap=ct.town.by.pop.rate.cap}

town.by.pop.rate.plt

```

Figure \@ref(fig:town-by-pop-rate10k) is similar to Figure \@ref(fig:town-by-pop-rate), but instead of showing the crude number of cases for each town, it shows the number of cases per 10,000 population.
...
```{r map-positivity, fig.height=7, fig.cap="Ten Day Average Covid-19 Test Positivity for each Connecticut Town."}
